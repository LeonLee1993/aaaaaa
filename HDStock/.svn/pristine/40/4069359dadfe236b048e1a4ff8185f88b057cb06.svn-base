...
- (NSMutableArray *)labelTitlesArr {
    if (!_labelTitlesArr) {
        _labelTitlesArr = [NSMutableArray array];
    }
    return _labelTitlesArr;
}
...
- (void)addTags:(NSArray *)tagStrs
{
    ...
    for (NSString *tagStr in tagStrs) {
        [self addTag:tagStr andIndex:index];
        index ++;
    }
}
// 添加标签
- (void)addTag:(NSString *)tagStr andIndex:(NSInteger) index
{
...
- (void)pan:(UIPanGestureRecognizer *)pan
{
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (otherButton) { // 插入到当前按钮的位置
            ...
            //重置更新后的标签title数组
            [_labelTitlesArr exchangeObjectAtIndex:i withObjectAtIndex:curI];
