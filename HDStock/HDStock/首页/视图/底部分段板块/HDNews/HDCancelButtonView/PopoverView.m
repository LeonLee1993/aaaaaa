...
//Height/width of the actual arrow 实际箭头的高度
#define kArrowHeight 12.f

//padding within the box for the contentView 用内容视图填充
#define kBoxPadding 0.f

//control point offset for rounding corners of the main popover box
#define kCPOffset 1.8f

//radius for the rounded corners of the main popover box //弹窗切圆度
#define kBoxRadius 10.f

//Curvature value for the arrow.  Set to 0.f to make it linear.//箭头的曲率
#define kArrowCurvature 6.f

//Minimum distance from the side of the arrow to the beginning of curvature for the box 从箭头侧边到弹窗弯曲开始的最小距离
#define kArrowHorizontalPadding 5.f

//Alpha value for the shadow 阴影的透明度
#define kShadowAlpha 0.4f

//Box gradient bg alpha //弹窗渐变背景的透明度
#define kBoxAlpha 0.95f

//Padding along top of screen to allow for any nav/status bars 到屏幕顶部的边距
#define kTopMargin 50.f

//margin along the left and right of the box 弹窗到屏幕左右边的边距
#define kHorizontalMargin 10.f


// DIVIDERS BETWEEN VIEWS 视图之间的分割栏

//Bool that turns off/on the dividers 是否显示视图间的分割器
#define kShowDividersBetweenViews YES

//color for the divider fill 分割线的填充颜色
#define kDividerColor [UIColor redColor]
//[UIColor colorWithRed:0.329 green:0.341 blue:0.353 alpha:0.15f]
...
@interface PopoverView ()<PopoverBaseViewDelegate>
...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(BaseView *)cView delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    [popoverView showAtPoint:point inView:view withContentView:cView];
    popoverView.delegate = delegate;
    cView.delegate = popoverView;
    return popoverView;
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _showDividerRects = kShowDividersBetweenViews;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text {
    ...
    CGSize textSize = [text boundingRectWithSize:CGSizeMake([[UIScreen mainScreen] bounds].size.width - kHorizontalMargin*4.f, 1000.f) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kTextFont} context:nil].size;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withViewArray:(NSArray *)viewArray {
    ...
    if(kShowDividersBetweenViews) {
        _dividerRects = [[NSMutableArray alloc] initWithCapacity:viewArray.count-1];
    }
    ...
    for(UIView *view in viewArray) {
        ...
        if(kShowDividersBetweenViews && i != viewArray.count-1) {
            ...
            [((NSMutableArray *)_dividerRects) addObject:[NSValue valueWithCGRect:dividerRect]];
        }
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray {
    ...
    for(UIView *view in viewArray) {
        ...
        //Center the views
        view.frame = CGRectMake(CGRectGetMinX(_boxFrame) + totalWidth*0.5f - view.frame.size.width*0.5f, view.frame.origin.y, view.frame.size.width, view.frame.size.height);
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(UIView *)cView {
    ...
    xOrigin = _arrowPoint.x - boxWidth*0.5f;
    ...
    if(topPoint.y - contentHeight - arrowHeight - topPadding < CGRectGetMinY(topViewBounds)) {
        ...
        _boxFrame = CGRectMake(xOrigin, _arrowPoint.y + arrowHeight, boxWidth, boxHeight);
    } else {
        //Position above.
        _above = YES;
        
        _boxFrame = CGRectMake(xOrigin, _arrowPoint.y - arrowHeight - boxHeight, boxWidth, boxHeight);
    }
...
- (void)showActivityIndicatorWithMessage:(NSString *)msg {
    ...
    if(subviewsArray && (subviewsArray.count > 0)) {
        ...
        if(_showDividerRects) {
            _showDividerRects = NO;
            [self setNeedsDisplay];
        }
    }
    ...
    _activityIndicator.frame = CGRectMake(CGRectGetMidX(contentView.bounds) - 10.f, CGRectGetMidY(contentView.bounds) - 10.f + 20.f, 20.f, 20.f);
...
- (void)drawRect:(CGRect)rect
{
    ...
    //Draw the title background
    {
        ...
        if(titleBGHeight > 0.f) {
            ...
            //In this case, the arrow is located between LT2 and RT1
            if(!_above) {
                [titleBGPath addLineToPoint:CGPointMake(_arrowPoint.x - kArrowHeight, yMin)];//left side
                [titleBGPath addCurveToPoint:_arrowPoint controlPoint1:CGPointMake(_arrowPoint.x - kArrowHeight + kArrowCurvature, yMin) controlPoint2:_arrowPoint];//actual arrow point
                [titleBGPath addCurveToPoint:CGPointMake(_arrowPoint.x + kArrowHeight, yMin) controlPoint1:_arrowPoint controlPoint2:CGPointMake(_arrowPoint.x + kArrowHeight - kArrowCurvature, yMin)];//right side
            }
    ...
    {
        if(kShowDividersBetweenViews && _showDividerRects) {
            if(_dividerRects && _dividerRects.count > 0) {
                for(NSValue *value in _dividerRects) {
...
- (void)popoverBaseviewButtonTouchUpsideDown:(UIButton *)button{

    if (button.tag == 1000) {
        [self dismiss];
        
        [self.delegate popoverViewDismiss:self.arrowPoint];
    }
}
