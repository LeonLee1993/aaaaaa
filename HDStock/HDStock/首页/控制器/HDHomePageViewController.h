//
//  HDHomePageViewController.h
//  HDStock
//
//  Created by hd-app02 on 16/11/9.
//  Copyright © 2016年 hd-app02. All rights reserved.
//

#import "HDStockBaseViewController.h"
#import "HDTopScrollCell.h"
#import "HDMiddleCell.h"

#import "HDScrollWordsTableViewCell.h"
#import "HDStockSayingCell.h"
#import "HDLoadFailCell.h"

#import "HDHeadLineBaseCell.h"
#import "HDHeadLineSingleViewCell.h"
#import "HDHeadLineBigViewCellCell.h"
#import "HDHeadLineThreeViewCellCell.h"

#import "HDSearchCenterViewController.h"
#import "HDInfoMationViewController.h"
#import "HDStockNavigationController.h"
#import "ProductDetailViewController.h"
#import "PersonalproductViewController.h"
#import "buiedProductListViewController.h"

#import "HeadLineViewController.h"
#import "HDHeadLineDetailViewController.h"
#import "HDAdvertisementModel.h"
#import "HDAdversementDetailViewController.h"
#import <SDCycleScrollView.h>

#import "PSYPopOutView.h"
#import "HDGetGoldenStock.h"
#import "HDHeaderView.h"
#import "PCMessageCenterViewController.h"
#import "HDSubjectViewController.h"
#import "HDGetGoldenStockSmallScreen.h"

#import "MyCollectedDataModel.h"
#import "UIView+PSYView.h"

#define HDTopCellHeight 264 * WIDTH

#define HDScrollWordsCellHeight 65 * WIDTH

#define HDMiddleCellHeight 197 * WIDTH

#define HDStockSayingCellHeight 152 * WIDTH

@interface HDHomePageViewController : HDStockBaseViewController

@end
