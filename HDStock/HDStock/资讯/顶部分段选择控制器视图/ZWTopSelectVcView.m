...
-(CGFloat)setupChildVcViewHeight
{
    if ([self.delegate respondsToSelector:@selector(childVcViewHeightInZWTopSelectVcView:)]) {
        ...
        return self.frame.size.height - TABBAR_HEIGHT;}
}
...
-(void)setupViewUnderContent
{
    ...
    self.viewUnder=[[UIView alloc]initWithFrame:CGRectMake(10, (_topViewHeight)-2, (_topViewWidth)/self.contentVC.childViewControllers.count - 20, 2)];
    self.viewUnder.backgroundColor = MAIN_COLOR;
...
-(void)setupContentViewContent
{
    ...
    self.childVcViewY=[self setupChildVcViewY];
     ...
     UIView *animationChangeView=[[UIView alloc]initWithFrame:CGRectMake(self.childVcViewX,self.childVcViewY, self.childVcViewWidth,self.childVcViewHeight)];
...
-(void)underViewMoveTo:(int)index{
    [UIView animateWithDuration:0.3 animations:^{
        self.viewUnder.frame=CGRectMake(index *((_topViewWidth)/self.index) + 10, (_topViewHeight)-2, (_topViewWidth)/self.index - 20, 2);
    }];
}
