...
/**
 当前绘制在屏幕上的数据源位置数组
 */
@property (nonatomic, strong) NSArray <YYLinePositionModel *>*drawLinePositionModels;
...
//                设置滑动速度
//                self.stockScrollView.decelerationRate = 0;
                [self.stockScrollView setNeedsDisplay];
                //绘制K线上部分
//                if(self.refreshFlag%3==0){
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    [self setNeedsDisplay];
    CGRect rect = self.stockScrollView.contentView.frame;
    rect.origin.x = scrollView.contentOffset.x;
    self.stockScrollView.contentView.frame = rect;
}
...
- (void)drawTopDesc {
    ...
    NSDictionary *attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:9],NSForegroundColorAttributeName:[UIColor YYStock_topBarNormalTextColor]};
    NSString *ma5Text = [NSString stringWithFormat:@"MA5 %.2f",[selectedModel MA5]];
    NSString *ma10Text = [NSString stringWithFormat:@"MA10 %.2f",[selectedModel MA10]];
    NSString *ma20Text = [NSString stringWithFormat:@"MA20 %.2f",[selectedModel MA20]];
...
- (void)updateDrawModels {
    ...
    CGFloat minma5 = [[[self.drawLineModels valueForKeyPath:@"MA5"] valueForKeyPath:@"@min.floatValue"] floatValue];
    CGFloat minma10 = [[[self.drawLineModels valueForKeyPath:@"MA10"] valueForKeyPath:@"@min.floatValue"] floatValue];
    CGFloat minma20 = [[[self.drawLineModels valueForKeyPath:@"MA20"] valueForKeyPath:@"@min.floatValue"] floatValue];
...
//    NSLog(@"min:%f min5:%f min10:%f  min20:%f",min,minma5,minma10,minma20);
    max = MAX(MAX(MAX(ma5, ma10), ma20), max);
    min = MIN(MIN(MIN(minma5, minma10), minma20), min);
    maxValue =  max;
    minValue =  min;
}
