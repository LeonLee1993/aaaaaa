...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.drawLineModels.count > 0) {
        if (!self.maskView || self.maskView.isHidden) {
            ...
            //绘制成交量
            self.volumeView.fiveDayKeyArr = self.fiveDayKeyArr;//5日键值
            self.volumeView.isFiveKLine = self.isFiveKLine;
            [self.volumeView drawViewWithXPosition:0 drawModels:self.drawLineModels];
            //更新背景线
            self.stockScrollView.isShowBgLine = YES;
            self.stockScrollView.isMinutes = !self.isFiveKLine;
...
- (void)drawLeftRightDesc {
    ...
    for (int i = 0; i < 3; i++) {
        float yValue;
        if(i==0){
            yValue=unit * i + YYStockScrollViewTopGap;
        }else if(i==1){
            yValue=unit * i + YYStockScrollViewTopGap - textSize.height/2.f;
        }else if(i==2){
            yValue=unit * i + YYStockScrollViewTopGap - textSize.height/2.f + topOffset;
        }
        
        NSString *text = [NSString stringWithFormat:@"%.2f",maxValue - unitValue * i];
        CGPoint leftDrawPoint = CGPointMake(leftGap ,yValue);
        ...
        CGPoint rightDrawPoint = CGPointMake(CGRectGetMaxX(self.stockScrollView.frame) - textSize2.width - 3, yValue);
...
- (void)updateScrollViewContentWidth {
    ...
    NSInteger minCount = self.isFiveKLine?self.drawLineModels.count:240;
