	...
	switch (self.labelLocation) {
		...
		case XYLabelLocationBottom: {
			[self.label mas_remakeConstraints:^(MASConstraintMaker *make) {
				...
				make.left.equalTo(self.mas_left).offset(offsetX);
				make.right.equalTo(self.mas_right).offset(- offsetX);
				make.bottom.equalTo(self.mas_bottom).offset(-(ScreenWidth/375*20.5));
				make.height.equalTo(@(height));
			}];			
			[self.imageView mas_remakeConstraints:^(MASConstraintMaker *make) {
				
				make.bottom.equalTo(self.label.mas_top).offset(-3);
				make.centerX.equalTo(self.mas_centerX);
				make.width.offset(ScreenWidth/375*50);
				make.height.offset(ScreenWidth/375*50);
